...
export class EmailService {
  ...
  async sendResetPasswordEmail(email: string, verificationLink: string): Promise<boolean> {
    const year = "2024"
    const template = await this.templateService.getTemplate("reset-password.html", { verificationLink, year });
  ...
  async sendVerificationEmail(email: string, verificationLink: string): Promise<boolean> {
    ...
    const year = "2024"
    const template = await this.templateService.getTemplate("verification.html", { verificationLink, year });
